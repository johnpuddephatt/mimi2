...
<div id="chatter" class="discussion">
    ...
    <div id="chatter_header" style="background-color:{{ $discussion->color }}">
        <div class="container">
            ...
            <h1>{{ $discussion->title }}</h1><span class="chatter_head_details">
                @lang('chatter::messages.discussion.head_details')<a class="chatter_cat"
    ...
    <div class="chatter-alert alert alert-danger">
        <div class="container">
            <p><strong><i class="chatter-alert-danger"></i> @lang('chatter::alert.danger.title')</strong>
                @lang('chatter::alert.danger.reason.errors')</p>
            <ul>
                ...
                <div class="col-md-9 right-column">
                            ...
                            <li data-id="{{ $post->id }}" data-markdown="{{ $post->markdown }}">
                                <span class="chatter_posts">
                                    ...
                                    <div id="delete_warning_{{ $post->id }}" class="chatter_warning_delete">
                                        ...
                                        <button
                                            class="btn btn-sm btn-danger pull-right delete_response">@lang('chatter::messages.response.yes_confirm')</button>
                                        <button
                                            class="btn btn-sm btn-default pull-right">@lang('chatter::messages.response.no_confirm')</button>
                                    </div>
                                    ...
                                    <div class="chatter_avatar">
                                        ...
                                        @if( (substr($post->user->{$db_field}, 0, 7) == 'http://') ||
                                        (substr($post->user->{$db_field}, 0, 8) == 'https://') )
                                        ...
                                        <span class="chatter_avatar_circle"
                                            style="background-color:#<?= \DevDojo\Chatter\Helpers\ChatterHelper::stringToColorCode($post->user->{Config::get('chatter.user.database_field_with_user_name')}) ?>">
                                    ...
                                    <div class="chatter_middle">
                                        <span class="chatter_middle_details"><a
                                                href="{{ \DevDojo\Chatter\Helpers\ChatterHelper::userLink($post->user) }}">{{ ucfirst($post->user->{Config::get('chatter.user.database_field_with_user_name')}) }}</a>
                                            <span
                    ...
                    <div id="new_response">
                        ...
                        <div class="chatter_avatar">
                            ...
                            <!-- If the user db field contains http:// or https:// we don't need to use the relative path to the image assets -->
                            @if( (substr(Auth::user()->{$db_field}, 0, 7) == 'http://') ||
                            (substr(Auth::user()->{$db_field}, 0, 8) == 'https://') )
                            ...
                            @else
                            <span class="chatter_avatar_circle"
                                style="background-color:#<?= \DevDojo\Chatter\Helpers\ChatterHelper::stringToColorCode(Auth::user()->{Config::get('chatter.user.database_field_with_user_name')}) ?>">
                        ...
                        <div id="new_discussion">
                            ...
                            <form id="chatter_form_editor" action="/{{ Config::get('chatter.routes.home') }}/posts"
                                ...
                                <div id="editor">
                                    ...
                                    <label
                                        id="tinymce_placeholder">@lang('chatter::messages.editor.tinymce_placeholder')</label>
                        ...
                        <div id="discussion_response_email">
                            <button id="submit_response" class="btn btn-success pull-right"><i class="chatter-new"></i>
                                @lang('chatter::messages.response.submit')</button>
                            @if(Config::get('chatter.email.enabled'))
                            <div id="notify_email">
                                <img src="{{ url('/vendor/devdojo/chatter/assets/images/email.gif') }}"
                                ...
                                <label class="switch">
                                    <input type="checkbox" id="email_notification" name="email_notification"
                                        @if(!Auth::guest() &&
                                        $discussion->users->contains(Auth::user()->id)){{ 'checked' }}@endif>
            ...
            <form id="chatter_form_editor_in_discussion_view"
                ...
                <div class="row">
                    <div class="col-md-7">
                        ...
                        <input type="text" class="form-control" id="title" name="title"
                            placeholder="@lang('chatter::messages.editor.title')" v-model="title"
                            value="{{ old('title') }}">
                    </div>
                ...
                <div id="new_discussion_footer">
                    <input type='text' id="color" name="color" /><span
                        class="select_color_text">@lang('chatter::messages.editor.tinymce_placeholder')</span>
                    <button id="submit_discussion" class="btn btn-success pull-right"><i class="chatter-new"></i> Create
